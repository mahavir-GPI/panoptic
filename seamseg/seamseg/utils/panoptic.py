    ...
    def __call__(self, sem_pred, bbx_pred, cls_pred, obj_pred, msk_pred, num_stuff):
        ...
        try:
            ...
            for msk_i, cls_i, obj_i in zip(msk_pred[idx], cls_pred[idx], obj_pred[idx]):
                # Check intersection
                #print("******************We are here********")
                #print(torch.Tensor(occupied).dtype, torch.Tensor(msk_i).dtype)
                occupied = torch.tensor(occupied, dtype=torch.uint8).to(torch.device("cuda"))
                msk_i = torch.tensor(msk_i, dtype=torch.uint8).to(torch.device("cuda"))
